...
def main():
    log.info("Retrieving trajectory sequences...")
    trajectories = client.sequences.list(
        metadata={"type": "Trajectory", "source": "EDM"}, limit=None
    )

    log.info(f"Found {len(trajectories)} trajectories")
    print(trajectories.to_pandas())

    asset_ids = sorted(list({x.asset_id for x in trajectories if x.asset_id is not None}))
    ...
    for trajectory_chunk in chunks(trajectories, 10):
        ...
        for trajectory in trajectory_chunk:
            assert isinstance(trajectory, Sequence)
            i += 1
            ...
            wellbore = assets_dict.get(trajectory.asset_id)
            if wellbore is None:
                continue
